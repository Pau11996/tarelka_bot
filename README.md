...
- **Вечерний:** вчера была запись, сегодня пусто, после 20:00 — «Сегодня в дневнике ТАРЕЛКИ пока пусто».
- **Стрик:** вместе с вечерним, если серия ≥ 2 — «Серия N дней сгорит, если сегодня не будет записи».
- **Реанимация:** ровно 3 дня без записи — «В дневнике ТАРЕЛКИ нет записей уже 3 дня».
- **Подписка:** за 2 дня до конца — «Подписка истекает ДД.ММ.ГГГГ. Продлить?».
- **Реферал:** приглашённый сделал первый анализ — «Начислено N бонусных AI-запроса».
- **Рассылка:** из админки, текст задаёт оператор (только с включёнными уведомлениями).

Отключение дневника и рассылок: `/notifications_off`. Напоминание о подписке не отключается.
